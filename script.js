...
async function setupAdminPage(){
  ...
  const loadApps=async(notify=false)=>{const previousPending=applications.filter(a=>a.status==="pending").map(a=>a.id);const {data,error}=await supabaseClient.from("whitelist_applications").select("id,user_id,minecraft_nickname,reason,status,created_at,profiles(username)").order("created_at",{ascending:false});if(error){$("admin-applications").innerHTML=`<div class="empty-state">${escapeHTML(error.message)}</div>`;return;}applications=data||[];renderApps();const newPending=applications.filter(a=>a.status==="pending"&&!previousPending.includes(a.id));if(notify&&newPending.length){showToast("Нова заявка Whitelist",`${newPending.length===1?"Надійшла нова заявка":"Надійшли нові заявки"}.`);}setPendingBadge(applications.filter(a=>a.status==="pending").length);};
