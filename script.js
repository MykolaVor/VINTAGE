...
async function setupAdminPage(){
  ...
  function openPlayer(id){const u=users.find(x=>x.id===id);if(!u)return;const app=latestWhitelist(id),muted=activeModeration(u,"mute"),banned=activeModeration(u,"ban");const body=$("player-profile-body");body.innerHTML=`<div class="player-modal-head"><div class="player-avatar large">${avatarLetter(u.username)}</div><div><div class="section-tag">PLAYER PROFILE</div><h2>${escapeHTML(u.username||"Без ніку")}</h2><span class="role-label">${u.role==="admin"?"ADMIN":"PLAYER"}</span></div></div><div class="player-profile-grid"><div class="player-stat"><span>Minecraft</span><strong>${escapeHTML(u.minecraft_nickname||"Не вказано")}</strong></div><div class="player-stat"><span>Email</span><strong>${escapeHTML(u.email||"—")}</strong></div><div class="player-stat"><span>Реєстрація</span><strong>${fmtDate(u.created_at)}</strong></div><div class="player-stat"><span>Модерація</span><strong>${banned?`BAN до ${fmtModeration(u.ban_until)}`:muted?`MUTE до ${fmtModeration(u.mute_until)}`:"Без обмежень"}</strong></div></div><div class="profile-whitelist"><strong>Остання Whitelist-заявка</strong><p>${app?`${escapeHTML(app.minecraft_nickname)} · ${escapeHTML(app.status.toUpperCase())} · ${fmtDate(app.created_at)}`:"Заявок ще немає."}</p></div><div class="player-profile-actions"><button class="small-button" data-mod="mute" data-id="${escapeHTML(id)}">${muted?"ЗМІНИТИ МУТ":"ВИДАТИ МУТ"}</button><button class="small-button danger-button" data-mod="ban" data-id="${escapeHTML(id)}">${banned?"ЗМІНИТИ БАН":"ЗАБАНИТИ"}</button>${muted?`<button class="small-button" data-clear="mute" data-id="${escapeHTML(id)}">ЗНЯТИ МУТ</button>`:""}${banned?`<button class="small-button danger-button" data-clear="ban" data-id="${escapeHTML(id)}">ЗНЯТИ БАН</button>`:""}</div>`;$("player-profile-modal")?.classList.remove("hidden")}
  ...
  $("player-profile-body")?.addEventListener("click",async e=>{const b=e.target.closest("button");if(!b)return;const id=b.dataset.id;if(b.dataset.clear){const {error}=await supabaseClient.rpc("admin_clear_moderation",{target_user:id,moderation_type:b.dataset.clear});if(error)showToast("Модерація",error.message,"error");else{showToast("Готово","Обмеження знято.");await loadUsers();openPlayer(id)}return}if(b.dataset.mod)openModeration(id,b.dataset.mod)});
